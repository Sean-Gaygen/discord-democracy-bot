...
class Voting{
	...
	static category_change(){
		...
		switch (Voting.category_select.value){
			...
			case "add_resolution":
				...
				Voting.generate_add_resolution();
				break;
	...
	static generate_post_form_blurb(){
		...
		if (Voting.from_submit && !Voting.sent_duplicate && !Voting.too_many_provisions){
			...
			let blurb = document.createElement("p");
			blurb.innerText = "Your provision has been accepted. Please allow up to 15 seconds to pass for your provision to be posted, I can't afford powerful servers yet.";
	...
	static generate_add_amend(){
		...
		text_box.setAttribute("maxlength", 1500); // TODO: remove magic number

		Voting.value1_div.appendChild(text_box);

		Voting.value2_div.setAttribute("dispaly", "block");
		Voting.value2_div.appendChild(Voting.generate_label("gug_again", "And the amendment shall be named; (optional, and will not show up in the constitution, only the poll)"));
		let name_box = document.createElement("textarea");
		name_box.setAttribute("name", "value2");
		name_box.setAttribute("maxlength", 300);

		Voting.value2_div.appendChild(name_box);
	...
	static generate_add_resolution(){

		Voting.value1_div.setAttribute("display", "block");

		Voting.value1_div.appendChild(Voting.generate_label("gug", "The following resolution shall be proposed to the voting public:"));
		Voting.value1_div.appendChild(document.createElement("br"));
		let text_box = document.createElement("textarea");
		text_box.setAttribute("name", "value1");
		...
		Voting.value2_div.setAttribute("dispaly", "block");
		Voting.value2_div.appendChild(Voting.generate_label("gug_again", "And the resolution/bill/act shall be named; (optional)"));
		let name_box = document.createElement("textarea");
		name_box.setAttribute("name", "value2");
		name_box.setAttribute("maxlength", 300);
